...
def visualize(file:str) -> None:
    ...
    if numEpisodes != 1:
        TD = TD.reshape((numEpisodes, -1)) # newShape = (numEps, time)

    # plot TD error, reward and cumulative reward
    _, axs = plt.subplots(3, 1, constrained_layout=True, sharex=True, figsize=(5,8))
    ...
    if numEpisodes != 1:
        Pl = Pl.reshape((numEpisodes, -1, 3)) # newShape = (numEps, time)
        Pl_noise = Pl_noise.reshape((numEpisodes, -1, 3)) # newShape = (numEps, time)
